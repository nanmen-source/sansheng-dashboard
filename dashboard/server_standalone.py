#!/usr/bin/env python3
"""
三省六部 · 看板独立部署版 API 服务器（无需 OpenClaw）
...
"""
import json, pathlib, subprocess, sys, threading, argparse, datetime, logging, re, os, hashlib, shutil, tempfile, fcntl
from http.server import BaseHTTPRequestHandler, HTTPServer
from urllib.parse import urlparse
from urllib.request import Request, urlopen

# ── 内联 file_lock 工具（无需外部依赖）──
def _lock_path(path):
    return pathlib.Path(path).parent / (pathlib.Path(path).name + '.lock')
...
# ── 内联 validate_url（无需外部依赖）──
def validate_url(url, allowed_schemes=('https',), allowed_domains=None):
    from urllib.parse import urlparse
    import ipaddress
    try:
        parsed = urlparse(url)
...
def _inline_refresh_live_data(tasks):
    """内联版 refresh_live_data，无需外部脚本。"""
    try:
        ...
        today_str_val = now_ts.strftime('%Y-%m-%d')
        for t in tasks:
            if t.get('state') in ('Doing', 'Assigned', 'Review'):
                updated_raw = t.get('updatedAt') or ''
                age_sec = None
                if updated_raw:
                    try:
                ...
                t['heartbeat'] = None
        today_done = sum(1 for t in tasks if t.get('state') == 'Done' and isinstance(t.get('updatedAt', ''), str) and t.get('updatedAt', '')[:10] == today_str_val)
        ...
        history = [{'at': t.get('updatedAt', '未知')[:19], 'official': t.get('official'), 'task': t.get('title'), 'out': t.get('output'), 'qa': '通过'} for t in tasks if t.get('state') == 'Done']
        payload = {
            ...
            'tasks': tasks,
            'history': history,
            'metrics': {'officialCount': len(officials), 'todayDone': today_done, 'totalDone': total_done, 'inProgress': in_progress, 'blocked': blocked},
            'syncStatus': {},
            'health': {'syncOk': True, 'syncLatencyMs': 0, 'missingFieldCount': 0},
        }
        atomic_json_write(DATA / 'live_status.json', payload)
    except Exception as e:
        log.warning(f'内联 refresh_live_data 失败: {e}')

def save_tasks(tasks):
    atomic_json_write(DATA / 'tasks_source.json', tasks)
    # 内联刷新 live_status.json（不依赖外部脚本）
    threading.Thread(target=_inline_refresh_live_data, args=(list(tasks),), daemon=True).start()
...
def read_skill_content(agent_id, skill_name):
    ...
    # 独立模式：从 data/skills/{agent_id}/{skill_name}/SKILL.md 读取
    skill_path = (DATA / 'skills' / agent_id / skill_name / 'SKILL.md').resolve()
    allowed_root = DATA.resolve()
    if not str(skill_path).startswith(str(allowed_root)):
    ...
    try:
        ...
        return {'ok': True, 'name': skill_name, 'agent': agent_id, 'content': skill_content, 'path': str(skill_path)}
    except Exception as e:
...
def add_skill_to_agent(agent_id, skill_name, description, trigger=''):
    ...
    # 独立模式：技能存储在 data/skills/{agent_id}/ 目录
    workspace = DATA / 'skills' / agent_id / skill_name
    ...
    # 独立模式：更新 agent_config.json 中的技能列表
    _sync_agent_skills_in_config(agent_id)
    return {'ok': True, 'message': f'技能 {skill_name} 已添加到 {agent_id}', 'path': str(skill_md)}

def _sync_agent_skills_in_config(agent_id):
    """独立模式：扫描 data/skills/{agent_id}/ 目录，更新 agent_config.json 中的技能列表。"""
    try:
        skills_dir = DATA / 'skills' / agent_id
        skills = []
        if skills_dir.exists():
            for skill_dir in skills_dir.iterdir():
                if skill_dir.is_dir():
                    ...
                    skill_info = {'name': skill_dir.name, 'path': str(skill_md)}
                    if source_json.exists():
                        try:
                            src = json.loads(source_json.read_text())
                            skill_info['description'] = src.get('description', '')
                            skill_info['sourceUrl'] = src.get('sourceUrl', '')
                        except Exception:
                            pass
        ...
        for ag in agents:
            if ag.get('id') == agent_id:
                ag['skills'] = skills
                break
        atomic_json_write(DATA / 'agent_config.json', cfg)
    except Exception as e:
        log.warning(f'_sync_agent_skills_in_config 失败: {e}')
...
def _get_agent_session_status(agent_id):
    """独立部署模式：返回空状态（无 OpenClaw session 文件）。"""
    return 0, 0, False


def _check_agent_process(agent_id):
    """独立部署模式：始终返回 False（无 openclaw-agent 进程）。"""
    return False


def _check_agent_workspace(agent_id):
    """独立部署模式：始终返回 True（视为已配置）。"""
    return True
...
def wake_agent(agent_id, message=''):
    """独立部署模式：唤醒功能需要 OpenClaw，此处返回提示信息。"""
    if not _SAFE_NAME_RE.match(agent_id):
        return {'ok': False, 'error': f'agent_id 非法: {agent_id}'}
    log.info(f'[独立模式] 唤醒 {agent_id} 请求已记录（需要 OpenClaw 才能实际执行）')
    return {'ok': False, 'error': '独立部署模式下无法唤醒 Agent，请在本地安装 OpenClaw 后使用完整功能'}
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    if not agent_id:
        ...
        return
    # 独立部署模式：记录派发意图但不实际调用 openclaw
    log.info(f'[独立模式] {task_id} 状态 {new_state} → 应派发给 {agent_id}（{trigger}），但 OpenClaw 未安装，跳过')
    _update_task_scheduler(task_id, lambda t, s: (
        s.update({
            'lastDispatchAt': now_iso(),
            'lastDispatchStatus': 'standalone-skip',
            'lastDispatchAgent': agent_id,
            'lastDispatchTrigger': trigger,
        }),
        _scheduler_add_flow(t, f'[独立模式] 状态已更新：{new_state}（需要 OpenClaw 才能自动派发 Agent）', to=_STATE_LABELS.get(new_state, new_state))
    ))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == '/api/morning-brief/refresh':
            ...
            def do_refresh_news():
                try:
                    news_script = SCRIPTS / 'fetch_morning_news.py'
                    if news_script.exists():
                        cmd = [sys.executable, str(news_script)]
                        if force:
                            cmd.append('--force')
                        subprocess.run(cmd, timeout=120)
                        push_to_feishu()
                    else:
                        log.warning('fetch_morning_news.py 不存在，跳过采集')
                except Exception as e:
                    log.warning(f'[refresh error] {e}')
            threading.Thread(target=do_refresh_news, daemon=True).start()
        ...
        if p == '/api/set-model':
            ...
            # 独立模式：直接更新 agent_config.json 中的模型配置
            def apply_model_standalone():
                try:
                    cfg = read_json(DATA / 'agent_config.json', {})
                    ...
                    for ag in agents_list:
                        if ag.get('id') == agent_id:
                            ag['model'] = model
                            ag['isDefaultModel'] = (model == default_model)
                            break
                    atomic_json_write(DATA / 'agent_config.json', cfg)
                    log_path = DATA / 'model_change_log.json'
                    log_data = read_json(log_path, [])
                    if isinstance(log_data, list):
                        log_data.append({'at': now_iso(), 'agentId': agent_id, 'newModel': model})
                        if len(log_data) > 200:
                            log_data = log_data[-200:]
                        atomic_json_write(log_path, log_data)
                    atomic_json_write(DATA / 'last_model_change_result.json', {
                        'at': now_iso(), 'applied': [{'agentId': agent_id, 'newModel': model}],
                        'errors': [], 'gatewayRestarted': False, 'rolledBack': False,
                    })
                    atomic_json_write(DATA / 'pending_model_changes.json', [])
                except Exception as e:
                    log.warning(f'set-model 独立模式失败: {e}')
            threading.Thread(target=apply_model_standalone, daemon=True).start()
            self.send_json({'ok': True, 'message': f'已更新: {agent_id} → {model}（独立模式，无需重启 Gateway）'})
        else:
            self.send_error(404)
...
def main():
    ...
    # 独立模式：跳过 startup_recover（不调用 openclaw）
